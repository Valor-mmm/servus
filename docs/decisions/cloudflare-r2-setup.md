...
dev-personal). The steps require a Cloudflare account with R2 enabled.
...
4. Name the bucket (e.g. `servus-photos-prod` for production,
   `servus-photos-dev` for personal dev).
...
5. Also note your **Cloudflare Account ID** (visible in the right sidebar of any
   dashboard page) → `R2_ACCOUNT_ID`.

## 4. Derive R2_PUBLIC_URL_BASE

The presigned URL base is:
...
| `R2_BUCKET`            | bucket name (e.g. `servus-photos-prod`)                  |
| `R2_PUBLIC_URL_BASE`   | `https://<account_id>.r2.cloudflarestorage.com/<bucket>` |

### Local dev (`.env` file)

Copy `.env.example` to `.env` and fill in the five R2 vars using your personal
dev bucket credentials. The `.env` file is git-ignored and must never be
committed.
...
Remove the five R2 env vars from Deno Deploy and revert the deploy. Any R2
