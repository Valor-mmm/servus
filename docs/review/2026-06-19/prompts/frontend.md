...
- **Signals**: correct `@preact/signals` usage; no stale closures; no
  unnecessary re-renders; state colocated sensibly.
...
- **Markup semantics & a11y**: real buttons/labels/landmarks; form fields
  labeled; alt text; focus management in interactive islands.
- **i18n compliance**: all user copy via `t()`; keys exist; no concatenated
  sentences that break translation.
- **CSS architecture** (`lib/styles`, `_app.tsx`): token usage, no
  dead/duplicated styles, responsive approach consistent, theme variables
  coherent.
- **Component quality**: prop typing, reuse vs duplication, oversized
  components.
...
components/ItemGroupsEditor, components/SchemaEditorForm,
components/SchemaFields; lib/styles + _app.tsx CSS architecture; i18n sweep
across routes+islands+ components; overall islands-vs-components hydration
audit.
