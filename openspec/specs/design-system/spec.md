...
The design system spec defines the visual language and interaction conventions
for the servus app: design tokens, theme switching, navigation, button and badge
variants, mascot usage, animation rules, layout sizing, and minimum touch
targets. The spec is theme-aware — it ships two complete themes (Raute light and
Sternenhimmel dark) wired through a token architecture that is designed to
accept a third theme later without component changes.
...
Interactive list-row controls (quantity increment and decrement buttons) MUST
have a minimum touch target height of 44 px on viewports < 768 px to meet iOS
