...
Defines authentication, session, CSRF, and rate-limiting requirements for the
servus app. Custom auth (no third-party providers) per CLAUDE.md §8; passwords
hashed with Argon2id; sessions are server-side records keyed by an opaque ID
delivered via a signed cookie.
