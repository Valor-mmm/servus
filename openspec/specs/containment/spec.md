...
Defines how items can be placed inside other items (containers). A container is
any item whose category has `canContain: true`. Containment is recursive to any
depth. The root-owns-room invariant ensures contained items never store a
`roomId` directly; their effective room is derived by walking the chain to the
root item.
