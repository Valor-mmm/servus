...
```
Routes replace their ad-hoc empty paragraphs/divs with `<EmptyState message={t("...")} />`.
...
`current.startsWith(href)` to `current === href || current.startsWith(href + "/")`.
This means `/items` only highlights on `/items` and `/items/123`, not
`/items/incomplete` (because `"/items/incomplete".startsWith("/items/")` is
true, which is correct — `/items/incomplete` IS under `/items`). Actually the
fix is already correct in `_app.tsx`; `BottomNav.tsx` needs the same treatment.
