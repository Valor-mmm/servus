...
- [x] 1.1 Add a test in `tests/unit/` (or extend an existing render test) that
      ...
      `.item-row-body`, `.item-row-top`, a truncating anchor inside
      `.item-row-top`, and a `.meta` span inside `.item-row-body`. The test MUST
      fail before the JSX change is made.
...
- [x] 2.1 Inside the `items.map` block, wrap the `<a>` and the optional
      ...
      `<div class="item-row-body">`. The thumbnail `<img>` and
      `<QuantityControl>` remain as direct children of `<li class="item-row">`.

## 3. Update CSS in `static/styles.css`

- [x] 3.1 Remove `flex-wrap: wrap` from `.item-row`; set `align-items: center`
      and `flex-wrap: nowrap`.
- [x] 3.2 Add `.item-row-body` rule:
      `flex: 1; min-width: 0; display: flex;
      flex-direction: column; gap: 0.15rem;`
- [x] 3.3 Add `.item-row-top` rule:
...
- [x] 3.4 Change the selector for the link styles from `.item-row a` to
      ...
      and the existing color/font-weight/hover rules.
- [x] 3.5 Add `.item-row-body .meta` rule:
      `font-size: 0.75rem;
...
- [x] 3.6 Inside the existing `@media (min-width: 768px)` block, add overrides
      for `.item-row-body`:
      `flex-direction: row; align-items: center; gap: 0.5rem;` and for
      `.item-row-body .meta`:
      `white-space: normal; overflow: visible;
      text-overflow: unset;` —
      this restores the single-line desktop layout.
